...
class CrochetApp:
    ...
    def select_video_file(self):
        ...
        if file_path:  # If a file was selected
            self.video_path_var.set(file_path)  # Update the video path variable
    ...
    def save_video_path(self):
        global FALLBACK_VIDEO_PATH
        FALLBACK_VIDEO_PATH = self.video_path_var.get().strip()
        print(f"Fallback video path updated to: {FALLBACK_VIDEO_PATH}")
    ...
    def setup_settings_tab(self):
        ...
        live_check = tk.Checkbutton(settings_frame, text="Live Mode", 
                                    variable=self.live_var, 
                                    font=('Comic Sans MS', 12),
                                    bg=THEMES[self.current_theme]["bg"], 
                                    fg=THEMES[self.current_theme]["fg"],
                                    selectcolor=THEMES[self.current_theme]["bg"],
                                    command=self.toggle_live_status)
        live_check.pack(anchor='w', pady=5)

        # Video Path Entry
        video_path_frame = tk.Frame(settings_frame, bg=THEMES[self.current_theme]["bg"])
        video_path_frame.pack(fill=tk.X, pady=10)

        tk.Label(video_path_frame, text="Fallback Video Path:", 
                font=('Comic Sans MS', 12),
                bg=THEMES[self.current_theme]["bg"], 
                fg=THEMES[self.current_theme]["fg"]).pack(side=tk.LEFT, padx=(0, 10))

        self.video_path_var = tk.StringVar(value=FALLBACK_VIDEO_PATH)
        video_path_entry = tk.Entry(video_path_frame, textvariable=self.video_path_var, 
                                    font=('Comic Sans MS', 12), width=30)
        video_path_entry.pack(side=tk.LEFT)

        # Button to open file dialog
        select_video_btn = tk.Button(video_path_frame, text="Browse", command=self.select_video_file,
                                    font=('Comic Sans MS', 12),
                                    bg=THEMES[self.current_theme]["button"], 
                                    fg=THEMES[self.current_theme]["button_text"])
        select_video_btn.pack(side=tk.LEFT, padx=10)

        # Save Video Path Button
        save_video_path_btn = tk.Button(video_path_frame, text="Save Path", command=self.save_video_path,
                                        font=('Comic Sans MS', 12),
                                        bg=THEMES[self.current_theme]["button"], 
                                        fg=THEMES[self.current_theme]["button_text"])
        save_video_path_btn.pack(side=tk.LEFT, padx=10)
    ...
    # Update the bot's live status
        bot.is_live = self.live_var.get()
        status = "LIVE" if bot.is_live else "OFFLINE"
        self.add_message_to_chat(f"Bot is now {status} 🔄")

    # Reinitialize video capture based on the mode
        if bot.is_live:
            self.cap = self.init_video_capture()  # Initialize webcam
        else:
        # Set the fallback video path
            self.cap = cv2.VideoCapture(self.video_path_var.get().strip())  # Use the specified video path

    # Check if the capture was successful
        if not self.cap.isOpened():
            print("Failed to open video source.")
        # Optionally, you can handle this case by showing a message in the GUI
        ...
        # Reinitialize video capture based on the mode
            ...
            # Reinitialize video capture based on the mode
                ...
                self.cap = self.init_video_capture()  # Reinitialize with the new mode
    ...
    def update_all_widget_styles(self):
        ...
        def update_widget_colors(widget):
            if isinstance(widget, (tk.Frame, tk.Label, tk.Button, tk.Entry, tk.Checkbutton, tk.Text, tk.Canvas)):
                try:
        ...
        for tab in [self.tab_live, self.tab_stats, self.tab_gallery, self.tab_settings]:
            update_widget_colors(tab)

        update_widget_colors(self.label_dialog)
    ...
    def toggle_live_status(self):
        ...
        if not self.cap.isOpened():
            print("Failed to open video source.")
            self.add_message_to_chat("Failed to open video source. Please check the path.")
